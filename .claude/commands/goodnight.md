...
6. **Build-changelog** — if we changed any files that affect **the project itself** (`*.mjs`, modes, templates, dashboard, configs — system-layer + notable user-layer tooling), append an entry to the build change-log per the **Contribution Change-Log convention** in `CLAUDE.md` (entry schema with **root-cause** + **Upstream:** flag; copy `templates/BUILD-CHANGELOG.template.md` → `BUILD-CHANGELOG.md` on first change). **Purely personal changes go in `career-log.md`, NOT here.** Skip if only routine data/memory files changed.
