...
Deno.serve(async (req) => {
  ...
  const { data: allowed, error: rlErr } = await supabase.rpc('check_rate_limit', { p_metric: 'apply-assist', p_limit: 20, p_window_seconds: 60 });
  if (rlErr) console.error('[rate_limit] apply-assist', rlErr); // fail-open: don't lock out paying users on a transient DB error
