 ...
 *
 * Usage:
 *   node engine/update-system.mjs check      # Check if update available
 *   node engine/update-system.mjs apply      # Apply update (after user confirms)
 *   node engine/update-system.mjs rollback   # Rollback last update
 *   node engine/update-system.mjs dismiss    # Dismiss update check
 *
...
const CANONICAL_REPO = 'https://github.com/mysticalsin/Hireloom.git';
const RAW_VERSION_URL = 'https://raw.githubusercontent.com/mysticalsin/Hireloom/main/VERSION';
const RELEASES_API = 'https://api.github.com/repos/mysticalsin/Hireloom/releases/latest';
...
async function apply() {
  ...
  try {
    ...
    console.log(`Rollback available: node engine/update-system.mjs rollback`);
...
function dismiss() {
  ...
  console.log('Update check dismissed. Run "node engine/update-system.mjs check" or say "check for updates" to re-enable.');
}
...
switch (cmd) {
  ...
  default:
    console.log('Usage: node engine/update-system.mjs [check|apply|rollback|dismiss]');
    process.exit(1);
}
