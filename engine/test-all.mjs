 ...
 *
 * Usage:
 *   node engine/test-all.mjs           # Run all tests
 *   node engine/test-all.mjs --quick   # Skip dashboard build (faster)
 */
...
const engineDirs = ['engine', 'engine/scan', 'engine/apply', 'engine/render', 'engine/tracker', 'engine/lib'];
const mjsFiles = engineDirs.flatMap(d => readdirSync(join(ROOT, d)).filter(f => f.endsWith('.mjs')).map(f => join(d, f)));
...
const scripts = [
  { name: 'engine/tracker/cv-sync-check.mjs', expectExit: 1, allowFail: true }, // fails without cv.md (normal in repo)
  { name: 'engine/tracker/verify-pipeline.mjs', expectExit: 0 },
  { name: 'engine/tracker/normalize-statuses.mjs', expectExit: 0 },
  { name: 'engine/tracker/dedup-tracker.mjs', expectExit: 0 },
  { name: 'engine/tracker/merge-tracker.mjs', expectExit: 0 },
  { name: 'engine/update-system.mjs check', expectExit: 0 },
];
...
try {
  const { classifyLiveness } = await import(pathToFileURL(join(ROOT, 'engine', 'scan', 'liveness-core.mjs')).href);
...
const systemFiles = [
  'CLAUDE.md', 'VERSION', 'docs/DATA_CONTRACT.md',
...
const allowedFiles = [
  // English README + localized translations (historical allowlist; references now purged)
