 ...
 *
 * Usage:
 *   node engine/apply/auto-apply.mjs                     → process all eligible packages
 *   node engine/apply/auto-apply.mjs --num 062           → process single package
 *   node engine/apply/auto-apply.mjs --dry-run           → fill but do NOT submit
 *   node engine/apply/auto-apply.mjs --threshold 4.0     → min score (default: 4.0; use 0 for all)
 *   node engine/apply/auto-apply.mjs --workers 1         → parallel instances (default: 1)
 *   node engine/apply/auto-apply.mjs --no-review         → skip the deterministic pre-submit validator
 *   node engine/apply/auto-apply.mjs --dump-fields       → (with --dry-run) list every extracted field
 *   node engine/apply/auto-apply.mjs --verbose           → detailed output
 *
...
const LOG_DIR      = join(PROJECT_DIR, 'engine', 'batch', 'logs');
...
const TMP_DIR      = join(PROJECT_DIR, 'engine', 'batch', 'tmp');
