...
| engine/render/generate-pdf.mjs | `engine/render/generate-pdf.mjs` | Para PDF |
...
13. Ejecuta:
```bash
node engine/render/generate-pdf.mjs \
...
```
engine/batch/tracker-additions/{{ID}}.tsv
```
...
**IMPORTANTE:** El orden TSV tiene status ANTES de score (col 5→status, col 6→score). En applications.md el orden es inverso (col 5→score, col 6→status). engine/tracker/merge-tracker.mjs maneja la conversión.
