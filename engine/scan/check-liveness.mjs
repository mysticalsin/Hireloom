 ...
 *
 * Usage:
 *   node engine/scan/check-liveness.mjs <url1> [url2] ...
 *   node engine/scan/check-liveness.mjs --file urls.txt
 *
...
async function main() {
  ...
  if (args.length === 0) {
    console.error('Usage: node engine/scan/check-liveness.mjs <url1> [url2] ...');
    console.error('       node engine/scan/check-liveness.mjs --file urls.txt');
    process.exit(1);
  }
