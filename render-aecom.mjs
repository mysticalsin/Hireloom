#!/usr/bin/env node
// render-aecom.mjs — render an AECOM CV (+ cover) from a content JSON using the
// Kimi-style engine (batch/tailor-engine.mjs: timeline layout, single-page tightening).
// Usage: node render-aecom.mjs "<content.json>" "<outDir>" "<baseName>" [breaktail]
...
if (!jsonPath || !outDir || !baseName) {
  console.error('Usage: node render-aecom.mjs "<content.json>" "<outDir>" "<baseName>" [breaktail]');
  process.exit(1);
}
