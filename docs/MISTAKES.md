...
- **Files affected:** `config/profile.yml` (overwritten, reconstructed); `apps/web/server.mjs` — added backup-before-overwrite to `/api/onboard/finalize`.
...
- **Files affected:** `apps/web/server.mjs` — `fillGreenhouseForm()` function
...
- **Files affected:** `apps/web/server.mjs` — verification check block (~line 605)
...
- **Files affected:** `apps/web/server.mjs` — waterfall submit loop (~line 857)
...
- **Files affected:** `apps/web/server.mjs` — autopilot loop (~lines 819, 909, 930)
...
- **Files affected:** `apps/web/server.mjs` — URL extraction (~lines 735, 745, 4176, 4186, 4335)
...
- **Files affected:** `apps/web/server.mjs` — added `validateFormBeforeSubmit()` supervisor function
...
real tracker backup auto-written by `engine/tracker/dedup-tracker.mjs`/`engine/tracker/normalize-statuses.mjs`
