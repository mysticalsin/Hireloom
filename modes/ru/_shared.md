...
<!-- ============================================================
     ...
     (engine/update-system.mjs). Обновления применяются вручную.
...
1b. **Первая оценка в сессии:** Запустить `node engine/tracker/cv-sync-check.mjs`. Если предупреждения — уведомить пользователя.
...
9. **Добавления в трекер как TSV** — не добавлять НОВЫЕ записи в `data/applications.md` напрямую; для новых записей писать TSV в `engine/batch/tracker-additions/` и делать merge. Прямое редактирование существующих строк допустимо для обновления `status`/`notes`.
...
`engine/render/generate-pdf.mjs` автоматически нормализует em-dash, умные кавычки и zero-width символы в ASCII для максимальной ATS-совместимости. Но лучше не генерировать их изначально.
