...
async function autoMergeTrackerAdditions() {
  try {
    const addDir = path.join(ROOT, 'engine', 'batch', 'tracker-additions');
    const files = await fs.readdir(addDir).catch(() => []);
    const pending = files.filter(f => f.endsWith('.tsv'));
    if (pending.length === 0) return;
    await new Promise((resolve) => {
      const proc = spawn('node', [path.join(ROOT, 'engine', 'tracker', 'merge-tracker.mjs')], { cwd: ROOT, stdio: 'ignore' });
...
<script>
  ...
  if (pathname === '/api/onboard/finalize' && req.method === 'POST') {
    try {
      ...
      const pdfProc = spawn('node', [path.join(ROOT, 'engine', 'render', 'generate-cv-pdf.mjs')], {
